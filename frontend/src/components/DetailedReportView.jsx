...
export default function DetailedReportView(props) {
  ...
  const model = useMemo(
    ...
    [school, scenario, inputs, report, prevReport, prevCurrencyMeta, programTypeProp]
  );
 ...
 console.log(model);
  ...
  const perfRows = useMemo(() => {
    ...
    const formatVariance = (value) => (isFiniteNumber(value) ? fmtPct(value) : "—");
    ...
    if (!base.length) {
      return [
        ...
        { metric: "Burs ve Indirimler", planned: "—", actual: "—", variance: "—" },
      ];
    }
    ...
    return base.map((row) => {
      ...
      const kind =
        metric === "Ogrenci Sayisi"
          ...
          ? "pct"
          : metric === "Gelirler" || metric === "Giderler" || metric === "Burs ve Indirimler"
          ? "money"
          : "number";
      return {
        metric,
  ...
  const onePagerKpis = useMemo(() => {
    ...
    return [
      ...
      { label: "Ortalama Ücret", value: isFiniteNumber(avgTuition) ? fmtMoneyKpi(avgTuition) : "—" },
      { label: "Toplam Gelir", value: isFiniteNumber(revTotal) ? fmtMoneyKpi(revTotal) : "—" },
      { label: "Toplam Gider", value: isFiniteNumber(expTotal) ? fmtMoneyKpi(expTotal) : "—" },
      ...
      <div>
        ...
        <Section title="Rakip Kurum (Özet)">
          <SimpleTable
            ...
            rows={formattedCompetitorRows.map((r, i) => ({ key: String(i), ...r }))}
          />
        </Section>
